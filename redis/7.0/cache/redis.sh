...
OPTIONS:
  ...
  --redisCachePassword  Redis cache password
...
Example: ${scriptName} --bindAddress 0.0.0.0 --redisCachePort 6379 --maxMemory 2048 --save no --allowSync no --syncAlias 12345 --psyncAlias 98765 --shutdownCommand /usr/local/bin/redis_shutdown
EOF
}
...
if [[ -z "${save}" ]]; then
  >&2 echo "No save flag specified!"
  exit 1
fi

if [[ -z "${allowSync}" ]]; then
  >&2 echo "No allow sync flag specified!"
  exit 1
fi

if [[ "${allowSync}" == "no" ]]; then
  if [[ -z "${syncAlias}" ]]; then
    >&2 echo "No sync alias specified!"
    exit 1
  fi

  if [[ -z "${psyncAlias}" ]]; then
    >&2 echo "No psync alias specified!"
    exit 1
  fi
fi
