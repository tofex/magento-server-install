...
Example: ${scriptName} --bindAddress 0.0.0.0 --redisFPCPort 6379 --maxMemory 512 --save no --allowSync no --syncAlias 12345 --psyncAlias 98765 --shutdownCommand /usr/local/bin/redis_shutdown
EOF
}
...
if [[ -z "${shutdownCommand}" ]]; then
  if [[ -n "${redisFPCPassword}" ]]; then
    shutdownCommand="\$CLIEXEC -p \$REDISPORT -a ${redisFPCPassword} --no-auth-warning shutdown"
  else
